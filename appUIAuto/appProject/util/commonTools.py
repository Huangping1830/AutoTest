...
from resultTools import createName
from appProject.util.readConf import myconf
from appProject.util.pathTools import root_path,apk_path,apk_name_path,conf_path
...
class CommonTools:
    ...
    def __init__(self, config):
        self._driver = None
        read_conf = myconf()
        config_name_path = conf_path + config
        read_conf.read(config_name_path)
        self.desired_caps = dict(read_conf.items("deviceConfig"))
        self.desired_caps["app"] = apk_name_path
    ...
    def saveScreenshot(self,pathOnPC=createName("image")):
    ...
    def sendKeys(self, elementInfo, keys):
        element = self.findElement(elementInfo)
        return element.send_keys(keys)

    """
    控件的size+location,滑动的四个数据,左右滑动
    """
    def swipeLocation(self,elementInfo):
        element = self.findElement(elementInfo)
        dict1=element.size
        dict2=element.location
        x1 = dict1['width'] - 50
        y1 = dict1['height'] / 2 + dict2['y']
        x2 = 50
        y2 = dict1['height'] / 2 + dict2['y']
        return x1,y1,x2,y2

    """
    控件的size+location,滑动的四个数据,上下滑动
    """
    def swipeLocationy(self,elementInfo):
        element = self.findElement(elementInfo)
        dict1=element.size
        dict2=element.location
        x1 = dict1["width"]/2
        y1 = dict1["height"] + dict2["y"]-150
        x2 = dict1["width"]/2
        y2 = 150
        return x1, y1, x2, y2